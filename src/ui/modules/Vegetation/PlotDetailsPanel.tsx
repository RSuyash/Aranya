...
export const PlotDetailsPanel: React.FC<PlotDetailsPanelProps> = ({ plotId, onClose }) => {
    ...
    if (loading) return <div className="p-8 text-[#9ba2c0]">Loading details...</div>;

    return (
        <div className="h-full flex flex-col bg-[#0b1020] text-[#f5f7ff] overflow-hidden">
            {/* Header */}
            <div className="px-6 py-4 border-b border-[#1d2440] flex items-center justify-between shrink-0 bg-[#050814]">
                <h2 className="text-lg font-bold flex items-center gap-2">
                    Plot Details
                    <span className="px-2 py-0.5 rounded-full bg-[#11182b] border border-[#1d2440] text-[10px] text-[#9ba2c0] font-normal">
                        {formData.code}
                    </span>
                </h2>
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-8 custom-scrollbar">
                ...
                <section className="space-y-4">
                    <div className="flex items-center gap-2 text-[#56ccf2] border-b border-[#1d2440] pb-2">
                        <MapPin className="w-4 h-4" />
                        <h3 className="text-sm font-semibold uppercase tracking-wider">Location & Geometry</h3>
                    </div>
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            <label className="block text-xs text-[#9ba2c0] mb-1">Latitude</label>
                            <input
                                type="number"
                                step="0.000001"
                                value={formData.coordinates?.lat || 0}
                                onChange={e => updateCoordinate('lat', parseFloat(e.target.value))}
                                className="w-full bg-[#050814] border border-[#1d2440] rounded-lg px-3 py-2 text-sm focus:border-[#56ccf2] outline-none"
                            />
                        </div>
                        <div>
                            <label className="block text-xs text-[#9ba2c0] mb-1">Longitude</label>
                            <input
                                type="number"
                                step="0.000001"
                        ...
                        <div>
                            <label className="block text-xs text-[#9ba2c0] mb-1">Altitude (m)</label>
                            <input
                                type="number"
                        ...
                        <div>
                            <label className="block text-xs text-[#9ba2c0] mb-1">GPS Accuracy (m)</label>
                            <input
                                type="number"
                ...
                {/* Section: Topography */}
                <section className="space-y-4">
                    <div className="flex items-center gap-2 text-[#52d273] border-b border-[#1d2440] pb-2">
                        <Mountain className="w-4 h-4" />
                        <h3 className="text-sm font-semibold uppercase tracking-wider">Topography</h3>
                    </div>
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            <label className="block text-xs text-[#9ba2c0] mb-1">Slope (degrees)</label>
                            <input
                                type="number"
                                value={formData.slope || 0}
                                onChange={e => setFormData({ ...formData, slope: parseFloat(e.target.value) })}
                                className="w-full bg-[#050814] border border-[#1d2440] rounded-lg px-3 py-2 text-sm focus:border-[#52d273] outline-none"
                            />
                        </div>
                        <div>
                            <label className="block text-xs text-[#9ba2c0] mb-1">Aspect</label>
                            <select
                                value={formData.aspect || 'N'}
                                onChange={e => setFormData({ ...formData, aspect: e.target.value })}
                                className="w-full bg-[#050814] border border-[#1d2440] rounded-lg px-3 py-2 text-sm focus:border-[#52d273] outline-none text-[#f5f7ff]"
                            >
                        ...
                        <div>
                            <label className="block text-xs text-[#9ba2c0] mb-1">Orientation (Azimuth)</label>
                            <input
                                type="number"
                ...
                {/* Section: Habitat */}
                <section className="space-y-4">
                    <div className="flex items-center gap-2 text-[#eebb4d] border-b border-[#1d2440] pb-2">
                        <Trees className="w-4 h-4" />
                        <h3 className="text-sm font-semibold uppercase tracking-wider">Habitat & Ecology</h3>
                    </div>
                    <div className="space-y-4">
                        <div>
                            <label className="block text-xs text-[#9ba2c0] mb-1">Habitat Type</label>
                            <input
                                type="text"
                                value={formData.habitatType || ''}
                                onChange={e => setFormData({ ...formData, habitatType: e.target.value })}
                                placeholder="e.g. Deciduous Forest"
                                className="w-full bg-[#050814] border border-[#1d2440] rounded-lg px-3 py-2 text-sm focus:border-[#eebb4d] outline-none"
                            />
                        </div>
                        <div>
                            <label className="block text-xs text-[#9ba2c0] mb-1">Disturbance Type</label>
                            <select
                                value={formData.disturbanceType || 'NONE'}
                                onChange={e => setFormData({ ...formData, disturbanceType: e.target.value as any })}
                                className="w-full bg-[#050814] border border-[#1d2440] rounded-lg px-3 py-2 text-sm focus:border-[#eebb4d] outline-none text-[#f5f7ff]"
                            >
                ...
                <section className="space-y-4">
                    <div className="flex items-center gap-2 text-[#ff7e67] border-b border-[#1d2440] pb-2">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        {[
                            ...
                            <div key={item.key}>
                                <label className="block text-xs text-[#9ba2c0] mb-1">{item.label}</label>
                                <div className="flex items-center gap-2">
                                    <input
                                        ...
                                        className="flex-1 accent-[#ff7e67]"
                                    />
            ...
            {/* Footer Actions */}
            <div className="p-6 border-t border-[#1d2440] bg-[#050814]">
                <button
                    onClick={handleSave}
                    disabled={saving}
                    className="w-full flex items-center justify-center gap-2 bg-[#56ccf2] text-[#050814] font-bold py-3 rounded-xl hover:bg-[#4ab8de] transition disabled:opacity-50"
                >
