                ...
                <div className="max-w-md mx-auto space-y-6">
                    {currentStep === 'ID' && (
                        <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
                            ...
                            <div>
                                <label className="block text-xs font-medium text-[#9ba2c0] uppercase mb-3">Growth Form *</label>
                                        ...
                                        <button
                                            ...
                                            className={clsx(
                                                ...
                                                growthForm === form.value
                                                    ? "border-[#56ccf2] bg-[#071824]"
                                                    : "border-[#1d2440] bg-[#11182b] hover:border-[#56ccf2]"
                                            )}
                                        >
                                            <div style={{ color: growthForm === form.value ? form.color : '#9ba2c0' }}>
                                                {form.icon}
                                            </div>
                                            <span className={clsx(
                                                "text-sm font-medium",
                                                growthForm === form.value ? "text-[#f5f7ff]" : "text-[#9ba2c0]"
                                            )}>
                                                {form.label}
                                            </span>
                                        </button>
                    ...
                    {currentStep === 'METRICS' && (
                        <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
                            <div>
                                <label className="block text-xs font-medium text-[#9ba2c0] uppercase mb-2">
                                    Individual Count <span className="text-[#555b75] lowercase">(optional)</span>
                                </label>
                                <input
                                    type="number"
                            ...
                            <div>
                                <label className="block text-xs font-medium text-[#9ba2c0] uppercase mb-2">
                                    Ground Cover (%) <span className="text-[#555b75] lowercase">(optional)</span>
                                </label>
                                <input
                                    type="number"
                                ...
                                {coverPercentage && parseFloat(coverPercentage) > 100 && (
                                    <p className="mt-2 text-xs text-[#ff7e67]">Cover percentage must be between 0-100</p>
                                )}
                            </div>

                            <div>
                                <label className="block text-xs font-medium text-[#9ba2c0] uppercase mb-2">
                                    Avg Height (cm) <span className="text-[#555b75] lowercase">(optional)</span>
                                </label>
                                <input
                                    type="number"
                                    value={avgHeight}
                        ...
                        <div className="space-y-4 animate-in fade-in slide-in-from-right-4 duration-300">
                            <button
                                ...
                                className={clsx(
                                    ...
                                    hasGroundPhoto ? "border-[#52d273] bg-[#0b2214]/30" : "border-[#1d2440] bg-[#11182b] hover:border-[#56ccf2]"
                                )}
                            >
                                <Camera className={clsx("w-8 h-8", hasGroundPhoto ? "text-[#52d273]" : "text-[#9ba2c0]")} />
                                <span className={clsx("font-medium", hasGroundPhoto ? "text-[#52d273]" : "text-[#9ba2c0]")}>
                            ...
                            <button
                                ...
                                className={clsx(
                                    "w-full p-6 rounded-xl border-2 border-dashed flex flex-col items-center gap-3 transition",
                                    hasCloseupPhoto ? "border-[#52d273] bg-[#0b2214]/30" : "border-[#1d2440] bg-[#11182b] hover:border-[#56ccf2]"
                                )}
                            >
                                <Camera className={clsx("w-8 h-8", hasCloseupPhoto ? "text-[#52d273]" : "text-[#9ba2c0]")} />
                                <span className={clsx("font-medium", hasCloseupPhoto ? "text-[#52d273]" : "text-[#9ba2c0]")}>
                        ...
                        <div className="space-y-4 animate-in fade-in slide-in-from-right-4 duration-300">
                            <div className="bg-[#11182b] border border-[#1d2440] rounded-xl p-4 space-y-3">
                                <div className="flex justify-between border-b border-[#1d2440] pb-2">
                                    <span className="text-[#9ba2c0]">Growth Form</span>
                                    <span className="font-medium text-[#f5f7ff]">{growthForm}</span>
                                </div>
                                <div className="flex justify-between border-b border-[#1d2440] pb-2">
                                ...
                                {abundanceCount && (
                                    <div className="flex justify-between border-b border-[#1d2440] pb-2">
                                        <span className="text-[#9ba2c0]">Count</span>
                                        <span className="font-mono text-[#f5f7ff]">{abundanceCount}</span>
                                    </div>
                                )}
                                {coverPercentage && (
                                    <div className="flex justify-between border-b border-[#1d2440] pb-2">
                                        <span className="text-[#9ba2c0]">Cover</span>
                                        <span className="font-mono text-[#f5f7ff]">{coverPercentage}%</span>
                                    </div>
                                )}
                                {avgHeight && (
                                    <div className="flex justify-between border-b border-[#1d2440] pb-2">
                                        <span className="text-[#9ba2c0]">Height</span>
                                        <span className="font-mono text-[#f5f7ff]">{avgHeight} cm</span>
                                    </div>
                                )}
                                <div className="flex justify-between">
                                    <span className="text-[#9ba2c0]">Photos</span>
                                    <span className="text-[#f5f7ff]">{[hasGroundPhoto, hasCloseupPhoto].filter(Boolean).length} added</span>
                                </div>
                            </div>
                        </div>
                    )}
