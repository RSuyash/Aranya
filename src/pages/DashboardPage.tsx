...
// --- 2. GRAPHICAL METRIC (Adaptive Glass) ---
const GraphicalMetric = ({ label, value, subLabel, icon: Icon, color, percent }: any) => (
    // UPGRADE: Replaced border-white/5 with border-border. Used bg-panel/40 for adaptive tint.
    <div className="relative overflow-hidden rounded-2xl bg-panel/40 backdrop-blur-xl border border-border p-5 flex items-center justify-between group hover:border-primary/30 transition-all duration-500 hover:shadow-lg hover:-translate-y-1 cursor-default">
        ...
        <div className="flex flex-col z-10 relative">
            ...
            <div className="flex items-baseline gap-2">
                ...
                <span className="text-[10px] font-bold text-text-muted bg-panel-soft px-1.5 py-0.5 rounded border border-border">{subLabel}</span>
            </div>

            <div className="h-1 w-16 bg-panel-soft rounded-full mt-3 overflow-hidden">
...
const ActiveMissionBanner = ({ data, onClick }: any) => (
    <div className="relative w-full overflow-hidden rounded-[20px] border border-border bg-panel shadow-2xl group cursor-pointer hover:border-primary/40 transition-colors duration-500">
        ...
        <div className="flex flex-col md:flex-row items-stretch min-h-[260px]">
            ...
            <div className="md:w-[300px] bg-panel-soft/50 backdrop-blur-md p-8 flex flex-col justify-between border-t md:border-t-0 md:border-l border-border relative">
                <div>
                    ...
                    <div className="relative h-1.5 w-full bg-panel rounded-full mt-4 overflow-hidden border border-border">
                        <div
                            ...
                            style={{ width: `${Math.min(100, (data.plots / 150) * 100)}%` }}
                        />
                    </div>
                ...
                <div className="grid grid-cols-2 gap-px bg-border rounded-lg overflow-hidden mt-6 border border-border">
                    <div className="bg-panel p-3 text-center hover:bg-panel-soft transition-colors">
...
const ModuleRow = ({ title, icon: Icon, status }: any) => (
    // UPGRADE: Removed bg-white/5. Used bg-panel/30 with border-transparent -> border-border.
    <div className="flex items-center justify-between p-4 rounded-xl border border-transparent bg-panel-soft/30 hover:bg-panel hover:border-border transition-all duration-300 cursor-not-allowed group">
        <div className="flex items-center gap-4">
            <div className="w-10 h-10 rounded-lg bg-panel-soft flex items-center justify-center text-text-muted group-hover:text-text-main transition-colors border border-border group-hover:border-primary/20">
        ...
        <div className="h-1.5 w-1.5 rounded-full bg-text-muted/20 group-hover:bg-primary transition-colors" />
    </div>
);
            ...
            <section className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                <div className="lg:col-span-8 space-y-6">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <button onClick={() => navigate('/map')} className="group p-6 rounded-[20px] bg-panel/40 border border-border hover:bg-panel hover:border-primary/30 transition-all duration-300 text-left relative overflow-hidden">
                            ...
                            <div className="mb-4 w-10 h-10 rounded-xl bg-bg-app border border-border flex items-center justify-center text-text-muted group-hover:text-primary group-hover:scale-110 transition-all shadow-inner">
                        ...
                        <button onClick={() => navigate('/analysis')} className="group p-6 rounded-[20px] bg-panel/40 border border-border hover:bg-panel hover:border-warning/30 transition-all duration-300 text-left relative overflow-hidden">
                            ...
                            <div className="mb-4 w-10 h-10 rounded-xl bg-bg-app border border-border flex items-center justify-center text-text-muted group-hover:text-warning group-hover:scale-110 transition-all shadow-inner">
                ...
                <div className="lg:col-span-4 space-y-6">
                    ...
                    <div className="space-y-3">
                        <ModuleRow title="Avian Surveys" icon={Bird} status="Beta Access" />
                        <ModuleRow title="Water Quality" icon={Droplets} status="Coming Soon" />
                        <ModuleRow title="Soil Horizons" icon={Mountain} status="Planned" />
                    </div>

                    <div className="mt-8 p-6 rounded-2xl border border-dashed border-border bg-panel-soft/30 text-center">
