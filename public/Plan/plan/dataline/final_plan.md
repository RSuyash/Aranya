# Data Pipeline: Field-to-Lab Architecture (Final Plan)
...
A single CSV file is insufficient. The Analyst Bundle will be a `.zip` file containing a relational schema:

*   **`data/plots.csv`**: Site-level metadata and geolocation.
*   **`data/sampling_units.csv`**: **[NEW]** Reference table for Subplots/Quadrants (Area, Dimensions) to calculate density.
*   **`data/trees.csv`**: Individual tree observations (Aggregated metrics).
*   **`data/stems.csv`**: **[NEW]** Raw stem measurements for multi-stem biomass calculations.
*   **`data/vegetation.csv`**: Non-tree vegetation data.
*   **`data/taxonomy.csv`**: **[UPDATED]** Union of Predefined List (Absence Data) AND Observed Species.
*   **`data/images.csv`**: **[NEW]** Log of captured media linked to parent records.
*   **`spatial/plots.geojson`**: FeatureCollection of plot centroids for GIS visualization.
*   **`metadata.json`**: Machine-readable project settings and protocol summary.
